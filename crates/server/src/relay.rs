//! Relais WebSocket : matchmaking, combats PvP et reproduction en ligne.
//!
//! Cette partie du serveur est sans état persistant : elle met en relation
//! deux joueurs et arbitre leur combat le temps de la connexion.
...
/// État partagé du relais (files d'attente en mémoire).
pub type RelayState = Arc<Mutex<ServerState>>;

/// Crée l'état du relais.
pub fn new_state() -> RelayState {
    Arc::new(Mutex::new(ServerState::new()))
}

/// Prend en charge une connexion WebSocket entrante.
pub async fn serve_connection(socket: TcpStream, peer: String, state: RelayState) {
