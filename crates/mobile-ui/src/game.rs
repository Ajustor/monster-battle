...
pub enum GameScreen {
    ...
    SelectAttacks,
    /// Gestion détaillée d'un monstre (stats, traits, actions).
    MonsterDetail,
    /// Sélection de la proie pour la dévoration manuelle.
    DevourSelect,
}
...
/// Index du monstre sélectionné pour l'écran MonsterDetail.
...
fn on_enter_devour_select(_data: ResMut<GameData>) {
    // DevourTargetIndex et DevourConfirmState sont gérés par monster_detail.
}
