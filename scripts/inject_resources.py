...
def main():
    ...
    if os.path.isdir(res_dir):
        for dp, _, fns in os.walk(res_dir):
            for f in fns:
                # Normaliser en séparateurs POSIX pour les entrées ZIP
                rel = os.path.relpath(os.path.join(dp, f), tmp)
                res_files.add(rel.replace(os.sep, "/"))

    # Files we want to replace from the overlay
    overlay_files = {"resources.arsc", "AndroidManifest.xml"} | res_files

    # Utilisation de context managers pour garantir la fermeture des ZipFile
    with zipfile.ZipFile(apk_src, "r") as src, \
         zipfile.ZipFile(apk_new, "w", zipfile.ZIP_DEFLATED) as dst:
        ...
        for rf in res_files:
            # rf utilise des séparateurs POSIX, reconstruire le chemin local
            local_path = os.path.join(tmp, rf.replace("/", os.sep))
            dst.write(local_path, rf)
